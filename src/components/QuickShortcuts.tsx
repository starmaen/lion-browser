          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Standalone Web App Mode indicator pin */}
                <button
                  id={`standalone-launch-${shortcut.id}`}
                  type="button"
                  title="فتح كتطبيق بدون ترويسة المتصفح"
                  onClick={(e) => {
                    e.stopPropagation();
                    onOpenShortcut(shortcut, true);
                  }}
                  className="absolute -top-1 -right-1 w-5 h-5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-full flex items-center justify-center shadow-md transition transform hover:scale-110 cursor-pointer border border-amber-300"
                >
