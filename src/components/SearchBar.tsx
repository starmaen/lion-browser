        ...
        {(Object.keys(SEARCH_ENGINES) as SearchEngineId[]).map((engineKey) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => onEngineChange(engineKey)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 cursor-pointer whitespace-nowrap ${
                isSelected
                  ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-black shadow-md shadow-amber-500/20 scale-105 border border-amber-300'
            ...
            >
              ...
              {isSelected && <span className="text-[10px] opacity-80">({engine.name})</span>}
            </button>
          );
        })}
      </div>
