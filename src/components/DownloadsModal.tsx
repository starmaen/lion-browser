...
export const DownloadsModal: React.FC<DownloadsModalProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'video' | 'document' | 'other'>('all');
  ...
  const filteredDownloads = downloads.filter((item) => {
    ...
    return item.fileType !== 'video' && item.fileType !== 'document';
  });
  ...
  const handleCaptureVideo = () => {
    ...
    setTimeout(() => {
      ...
      const newItem: DownloadItem = {
        id: 'dl-' + Date.now(),
        fileName: `${cleanName}_${selectedQuality}.${selectedQuality === 'audio' ? 'mp3' : 'mp4'}`,
        fileType: selectedQuality === 'audio' ? 'audio' : 'video',
        fileSize: sizeMap[selectedQuality],
        progress: 100,
        status: 'completed',
        sourceUrl: currentUrl || 'https://www.youtube.com',
        downloadDate: 'الآن',
  ...
  const handleStartCustomDownload = (e: React.FormEvent) => {
    ...
    const newItem: DownloadItem = {
      id: 'dl-' + Date.now(),
      fileName: customFileName.trim() || 'ملف_تنزيل_' + Date.now(),
